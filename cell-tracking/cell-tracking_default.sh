...
ILP=/export/home/lparcala/Fluo-N2DH-SIM/01/tracking-2015-02-27.ilp  #many ilps in the folder, took one of it
...
python hypotheses_graph_to_json.py --method=conservation --without-tracklets --max-nearest-neighbors=1 --json-output ${JSON_GRAPH} --max-number-objects=2 --raw-data-file ${RAW_FILE} --raw-data-path data --labelImgPath '/TrackingFeatureExtraction/LabelImage/0/[[%d, 0, 0, 0, 0], [%d, %d, %d, %d, 1]]' ${ILP} #--transition-classifier-filename ${TRANSITION_CLASSIFIER_FILE} 
